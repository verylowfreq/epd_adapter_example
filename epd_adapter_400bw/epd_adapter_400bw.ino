...
constexpr uint16_t PANEL_W = GxEPD2_DRIVER_CLASS::WIDTH;
constexpr uint16_t PANEL_H = GxEPD2_DRIVER_CLASS::HEIGHT;
constexpr size_t IMAGEBUF_SIZE_W_H = ((PANEL_W + 7) / 8) * PANEL_H;
constexpr size_t IMAGEBUF_SIZE_H_W = ((PANEL_H + 7) / 8) * PANEL_W;
// constexpr size_t IMAGEBUF_SIZE = (IMAGEBUF_SIZE_W_H > IMAGEBUF_SIZE_H_W) ? IMAGEBUF_SIZE_W_H : IMAGEBUF_SIZE_H_W;
constexpr size_t IMAGE_DRAW_HEIGHT = 100;
constexpr size_t IMAGEBUF_SIZE = ((PANEL_W + 7) / 8) * IMAGE_DRAW_HEIGHT;
...
void process_stream_input_binary(Stream& stream) {
  ...
  if (cmd == 0x80) {
    ...
    const uint16_t draw_h = min<uint16_t>(IMAGE_DRAW_HEIGHT, image_height() - draw_cursor_y);
    display.firstPage();
    display.setPartialWindow(0, draw_cursor_y, image_width(), IMAGE_DRAW_HEIGHT);
    do {
    ...
    uint8_t resp[] = {
      ...
      7,     // Data length
      (uint8_t)(w & 0xff), (uint8_t)(w >> 8),   // Width as u16le
      (uint8_t)(h & 0xff), (uint8_t)(h >> 8),   // Height as u16le
      1,                               // Bit per pixel
      (uint8_t)(IMAGE_DRAW_HEIGHT & 0xff), (uint8_t)(IMAGE_DRAW_HEIGHT >> 8),  // Unit of height in one draw
    };
...
void process_serial_input() {
  if (Serial.available() > 0) {
    ...
    if (input.equals("clear")) {
      ...
      do {
        display.fillScreen(GxEPD_WHITE);
        display.drawBitmap(0, 0, imagebuf, image_width(), image_height(), GxEPD_BLACK);
